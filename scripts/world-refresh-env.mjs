...
const signingKey = getEnv('WORLD_ID_SIGNING_KEY') ?? getEnv('RP_SIGNING_KEY');
const envPath = path.resolve(process.cwd(), '.env.local');

if (!rpId) {
  console.error('Missing VITE_WORLD_ID_RP_ID.');
  console.error(
    'Usage: VITE_WORLD_ID_RP_ID=rp_xxx WORLD_ID_SIGNING_KEY=sk_xxx npm run world:refresh-env -- <action>',
  );
  console.error('You can also place VITE_WORLD_ID_RP_ID and WORLD_ID_SIGNING_KEY in .env.local.');
  process.exit(1);
}

if (!signingKey) {
  console.error('Missing WORLD_ID_SIGNING_KEY.');
  console.error(
    'Usage: VITE_WORLD_ID_RP_ID=rp_xxx WORLD_ID_SIGNING_KEY=sk_xxx npm run world:refresh-env -- <action>',
  );
  console.error('You can also place VITE_WORLD_ID_RP_ID and WORLD_ID_SIGNING_KEY in .env.local.');
